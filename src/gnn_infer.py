...
def get_project_root() -> Path:
    ...
    Assumption: this file lives in `<project_root>/src/gnn_infer.py`.
    """
...
class MahalanobisAD:
    ...
    """
    def __init__(self, ad_dir: PathLike):
        self.ad_dir = str(ad_dir)

        ad_dir_path = Path(ad_dir)
        self.mu = np.load(ad_dir_path / "md_mu.npy")                      # (1024,)
        self.inv_cov = np.load(ad_dir_path / "md_inv_cov.npy")            # (1024,1024)
        self.thr = float(np.load(ad_dir_path / "md_thr.npy")[0])          # scalar
        self.median = np.load(ad_dir_path / "md_median.npy")
        self.iqr = np.load(ad_dir_path / "md_iqr.npy")
...
def load_predictor_generic(
    dataset_name: str,
    save_root: Optional[PathLike] = None,
    ckpt_name: str = "checkpoint.pt",
    device: Optional[torch.device] = None,
    ad_dir: Optional[PathLike] = None,
) -> GenericGNNPredictor:
    project_root = get_project_root()
    save_root_path = Path(save_root) if save_root is not None else (project_root / "models")
    ckpt_path = save_root_path / dataset_name / ckpt_name

    ad_dir_path: Optional[Path] = None
    if ad_dir is not None:
        ad_dir_path = Path(ad_dir)

        # If a relative path is passed, interpret it as project-root relative.
        if not ad_dir_path.is_absolute():
            ad_dir_path = project_root / ad_dir_path

    return GenericGNNPredictor(ckpt_path=str(ckpt_path), device=device, ad_dir=str(ad_dir_path) if ad_dir_path else None)
