...
# 推論ユーティリティ（GenericGNNPredictor + load_predictor_generic）
from src.gnn_infer import load_predictor_generic
    ...
    def __init__(self):
        ...
        try:
            self.pred_zinc = load_predictor_generic(
                dataset_name="zinc",
                save_root=PROJECT_ROOT / "models",
                ckpt_name="checkpoint.pt",
                ad_dir=PROJECT_ROOT / "models" / "ad_artifacts" / "zinc",
                )
        ...
        try:
            self.pred_nrel = load_predictor_generic(
                ...
                save_root=PROJECT_ROOT / "models",
                ckpt_name="checkpoint.pt",
                ad_dir=PROJECT_ROOT / "models" / "ad_artifacts" / "nrel",
                )
        except Exception as e:
            self.pred_nrel = None
